...
const validateBoundaryExcelContent = async (file, t) => {
    return new Promise((resolve) => {
      ...
      reader.onload = (event) => {
        ...
        if (headers.length === 0) {
          resolve({ success: false, error: t("The sheet is empty or does not have headers.") });
          return;
        }
        ...
        rows.forEach((row, rowIndex) => {
          ...
          if (row[0]?.trim() && row[0]?.trim() !== referenceCountry) {
            errors.push(`${t("Row")} ${rowIndex + 2}: ${t("Country in the first column is inconsistent.")}`);
          }
          ...
          for (let colIndex = 1; colIndex < validateColumnsCount; colIndex++) {
            ...
            if (currentCell && !previousCell) {
              errors.push(
                `${t("Row")} ${rowIndex + 2}: ${t("Column")} "${headers[colIndex]}" ${t("is filled but")} "${headers[colIndex - 1]}" ${t("is empty.")}`
              );
            }
          }
        });
