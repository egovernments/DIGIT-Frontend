...
function AppConfigurationWrapper({ screenConfig, localeModule }) {
    ...
    const validateValue = (value, validation, label, a, b) => {
      ...
      if (validation.required) {
        if (
          ...
          return validation.message || `${t(`${label || "FIELD"}_REQUIRED_FOR`)} ${useCustomT(a?.label)}`;
        }
      }
  ...
  return (
    <AppConfigContext.Provider value={{ state, dispatch, openAddFieldPopup }}>
      ...
      <div className="app-config-flex-container" style={{ display: "flex", alignItems: "flex-end" , gap: "5rem" }}>
        <Button
