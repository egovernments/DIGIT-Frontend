import { Loader, FormComposerV2, Menu, ActionBar, SubmitBar } from "@egovernments/digit-ui-react-components";
...
const SetupCampaign = ({ hierarchyType, hierarchyData }) => {
  ...
  const { isLoading, data: projectType } = Digit.Hooks.useCustomMDMS(tenantId, "HCM-PROJECT-TYPES", [{ name: "projectTypes" }],{}, { schemaCode: `${"HCM-PROJECT-TYPES"}.projectTypes` });
    ...
    <React.Fragment>
      ...
      {actionBar === "true" && (
        <ActionBar style={{ zIndex: "19" }}>
          {displayMenu ? <Menu options={["HCM_UPDATE_DATES", "HCM_CONFIGURE_APP", "HCM_UPDATE_CAMPAIGN"]} t={t} onSelect={onActionSelect} /> : null}
