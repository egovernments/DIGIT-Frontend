...
import { Dropdown, Card, LabelFieldPair, TextInput, Button } from "@egovernments/digit-ui-components";
...
function DependentFieldsWrapper({ t, parentState, onExpressionChange, screenConfig, selectedFieldItem }) {
  ...
  const ALL_OPERATOR_OPTIONS = [
    { code: '==',  name: t('equals to') || 'equals to' },
    { code: '!==', name: t('not equals to') || 'not equals to' },
    { code: '>=',  name: t('greater than or equals to') || 'greater than or equals to' },
    { code: '<=',  name: t('less than or equals to') || 'less than or equals to' },
    { code: '>',   name: t('greater than') || 'greater than' },
    { code: '<',   name: t('less than') || 'less than' },
  ];
