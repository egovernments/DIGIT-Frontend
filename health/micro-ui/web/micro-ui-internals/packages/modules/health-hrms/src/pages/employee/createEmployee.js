...
const CreateEmployee = () => {
 ...
 const { data: mdmsData,isLoading } = Digit.Hooks.useCommonMDMS(Digit.ULBService.getStateId(), "egov-hrms", ["CommonFieldsConfig", "CampaignAssignmentFieldsConfig"], {
    select: (data) => {
      return {
  ...
  const validatePassword = (formData) => {
      ...
      return password && confirmPassword && password.match(Digit.Utils.getPattern('Password')) &&
          confirmPassword.match(Digit.Utils.getPattern('Password')) && (password === confirmPassword);
  }
  useEffect(() => {
    if (mobileNumber && (mobileNumber.length === 9 || mobileNumber.length === 10) &&
        (mobileNumber.match(Digit.Utils.getPattern('MobileNo')) ||
            mobileNumber.match(Digit.Utils.getPattern('MozMobileNo')))) {
      setShowToast(null);
      setPhonecheck(true);
  ...
  const onFormValueChange = (setValue = true, formData) => {
    ...
    if (formData?.SelectEmployeePhoneNumber?.mobileNumber) {
        setMobileNumber(formData?.SelectEmployeePhoneNumber?.mobileNumber?.startsWith(HRMS_CONSTANTS.INDIA_COUNTRY_CODE) ?
  ...
  const navigateToCampaignAssignmentAcknowledgement = (ProjectStaffPayload, selectedCampaignBoundary) => {
      Digit.HRMSService.search(tenantId, null, { codes: hrmsUserName }).then((res) => {
      ...
      if (!assignCampaigns) {
        ...
        for (let i = 0; i < assignedCampaignData?.length; i++) {
            ProjectStaffCreatePayload.push({
                ...
                startDate: assignedCampaignData?.[i]?.fromDate ? convertDateToEpoch(assignedCampaignData?.[i]?.fromDate) : assignedCampaignData?.[i].selectedProject?.startDate,
                endDate: assignedCampaignData?.[i]?.toDate != null 
