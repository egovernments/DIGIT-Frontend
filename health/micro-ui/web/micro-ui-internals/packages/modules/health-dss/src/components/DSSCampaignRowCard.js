...
const getActionButtons = (rowData, tabData, history, t, boundaryCodeResponse, campaignData) => {
  ...
  actions.dashboardLink = {
    ...
    onClick: () =>
      history.push(
        `/${window?.contextPath}/employee/dss/view-dashboard?campaignNumber=${rowData?.referenceID}&boundaryType=${rowData?.address?.boundaryType?.toLowerCase()}&boundaryValue=${boundaryValue}&boundaryCode=${rowData?.address?.boundary}`,
        {
          project: rowData,
