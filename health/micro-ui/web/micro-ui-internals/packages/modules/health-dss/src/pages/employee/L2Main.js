...
const overrideDescendantDateRange = (province) => {
  ...
  let descendantDateRange = Digit.SessionStorage.get("descendantDateRange");
  if (dssFilters?.filters && dssFilters?.range && descendantDateRange?.[province]) {
    dssFilters["filters"]["campaignStartDate"] = descendantDateRange[province]?.startDate?.toString();
    dssFilters["filters"]["campaignEndDate"] = descendantDateRange[province]?.endDate?.toString();
    dssFilters["range"]["startDate"] = new Date(descendantDateRange[province]?.startDate)?.toISOString();
    dssFilters["range"]["endDate"] = new Date(descendantDateRange[province]?.endDate)?.toISOString();
...
function getProjectTypeIDFromURL() {
  ...
  const matchingProject = projectTypes?.find(item => item?.dashboardUrls && 
    Object.values(item?.dashboardUrls)?.some(dashboardUrl => url === dashboardUrl)
  );
...
const L2Main = ({}) => {
  ...
  const [filters, setFilters] = useState(() => {
    const { startDate, endDate, title, interval, denomination, dateFilterSelected, tenantId, moduleLevel, boundaries, province, district } = getInitialRange();
    return {
      denomination,
      dateFilterSelected,
  ...
  const downloadOptions = [
    ...
    //   icon: <DownloadImageIcon />,
      ...
      onClick: () => {
        ...
        setTimeout(() => {
          console.log("Downloading Image Home");
      ...
      <div ref={fullPageRef} id="divToPrint">
        <div className="options">
          <Header styles={mobileView ? { marginLeft: "0px", whiteSpace: "pre-line" } : { marginBottom: "0px", whiteSpace: "pre" }}>
            {t(dashboardConfig?.[0]?.name)} {boundaryName && <span style={{ fontSize: "24px" }}>({t(getTitleHeading(boundaryName))})</span>}
          </Header>
          {mobileView ? null : (
