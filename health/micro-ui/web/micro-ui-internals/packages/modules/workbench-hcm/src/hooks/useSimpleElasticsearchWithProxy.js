...
import { createProxyAwareWorkerString } from "../workers/simpleElasticsearchWorkerWithProxy";
import { getKibanaDetails } from "../utils/getProjectServiceUrl";
import { getElasticProxyConfig } from "../utils/elasticProxyConfig";

/**
 * Simple Elasticsearch Hook with Proxy Support
 * 
 * This hook automatically detects if it should use a proxy server
 * or connect directly to Elasticsearch/Kibana.
 * 
 ...
 * @param {boolean} config.useProxy - Force use of proxy server (default: auto-detect)
 * @param {string} config.kibanaPath - Kibana path (default: from config or 'kibana')
...
const useSimpleElasticsearchWithProxy = (config) => {
  // Get proxy configuration
  const proxyConfig = getElasticProxyConfig(config.useProxy);
  
  // Get authentication details
  const user = getKibanaDetails('BasicUsername');
  const pass = getKibanaDetails('BasicPassword');
  const auth = user && pass ? btoa(`${user}:${pass}`) : '';
  const basicAuth = auth ? `Basic ${auth}` : '';
  ...
  const {
    ...
    maxBatchSize = 10000,
    parallelBatches = 4,
    kibanaPath = proxyConfig.useProxy ? proxyConfig.kibanaPath : (getKibanaDetails('kibanaPath') || 'kibana'),
    authKey = basicAuth,
    username = getKibanaDetails('username'),
    password = getKibanaDetails('password'),
    requiresAuth = !proxyConfig.useProxy, // Don't require auth in proxy mode
    enabled = true,
    autoFetch = true
  } = config;
  ...
  const [metadata, setMetadata] = useState({
    ...
    batchesProcessed: 0,
    config: null,
    usingProxy: proxyConfig.useProxy
  });
  ...
  // Initialize worker with proxy support
  useEffect(() => {
    if (!enabled) return;

    try {
      // Create worker with proxy-aware code
      const workerString = createProxyAwareWorkerString(proxyConfig.useProxy);
      ...
      console.log(`✅ Elasticsearch worker initialized (Proxy: ${proxyConfig.useProxy})`);
  ...
  const fetchData = useCallback(async (customConfig = {}) => {
    ...
    // Validate authentication parameters if required (not in proxy mode)
    if (!proxyConfig.useProxy && requiresAuth && (!username || !password)) {
    ...
    const fetchConfig = {
      ...
      maxBatchSize,
      parallelBatches,
      kibanaPath: proxyConfig.useProxy ? '' : kibanaPath, // Empty for proxy mode
      authKey: proxyConfig.useProxy ? authKey : authKey, // Pass auth for proxy to forward
      username: proxyConfig.useProxy ? null : username, // Don't send credentials in proxy mode
      password: proxyConfig.useProxy ? null : password,
      requiresAuth: !proxyConfig.useProxy && requiresAuth,
      origin: window.location.origin,
      requestId: requestIdRef.current,
      ...customConfig
    };

    console.log(`🚀 Starting fetch with config (Proxy: ${proxyConfig.useProxy}):`, fetchConfig);
  ...
  }, [indexName, query, sourceFields, maxRecordLimit, maxBatchSize, parallelBatches, kibanaPath, authKey, username, password, requiresAuth, proxyConfig.useProxy]);
  ...
  useEffect(() => {
    if (enabled && autoFetch && indexName && query && sourceFields) {
      ...
      if (!proxyConfig.useProxy && requiresAuth) {
  ...
  }, [enabled, autoFetch, indexName, JSON.stringify(query), JSON.stringify(sourceFields), authKey, username, password, requiresAuth, proxyConfig.useProxy]);
...
export default useSimpleElasticsearchWithProxy;