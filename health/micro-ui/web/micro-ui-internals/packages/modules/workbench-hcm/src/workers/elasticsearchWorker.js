    ...
    for (let batch = 0; batch < totalBatches; batch++) {
        ...
        if (queryParams && typeof queryParams === 'object') {
          Object.keys(queryParams).forEach(function(field) {
            ...
            if (value !== null && value !== undefined && value !== '') {
              ...
              const queryType = fieldConfig?.queryType || kibanaConfig.queryType || 'term';
        ...
        if (kibanaConfig.additionalFilters) {
          if (elasticsearchQuery.query.bool) {
            elasticsearchQuery.query.bool.must = [
              ...elasticsearchQuery.query.bool.must,
              ...kibanaConfig.additionalFilters
            ];
          } else {
      ...
      if (data && data.hits && data.hits.hits) {
        ...
        if (kibanaConfig.dataParser) {
          // Use custom data parser if provided
          batchData = data.hits.hits.map(function(hit, index) {
            return kibanaConfig.dataParser(hit, batchOffset + index);
          });
        } else {
