...
const BoundarySelection = ({
  ...
  const reqCriteria = {
    ...
    config: {
      select: (data) => {
        const result = { hierarchy: { ...data?.BoundaryHierarchy?.[0] }, hierarchyType: "Workbench" };
        setParams(result);
        return result;
      },
    },
  ...
  const { data: hierarchyDefinition, isLoading: isLoadingHierarchyDef } = Digit.Hooks.useCustomAPIHook(reqCriteria);
  ...
  if (isLoadingHierarchyDef) {
    return <LoaderWithGap text={"CAMPAIGN_BOUNDARY_PLEASE_WAIT"}></LoaderWithGap>;
  }
