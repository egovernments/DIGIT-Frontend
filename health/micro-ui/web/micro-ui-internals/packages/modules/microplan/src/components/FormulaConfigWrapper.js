...
const FormulaConfigWrapper = ({onSelect, props:customProps})=>{
   ...
   const [formulaConfigValues, setFormulaConfigValues] = useState(customProps?.sessionData?.FORMULA_CONFIGURATION?.formulaConfiguration?.formulaConfigValues || [])
...
const filteredAutoFilledRuleConfigurations = state.AutoFilledRuleConfigurations?.filter((item) => {
   ...
   return (
    ...
    const ruleConfigurationCategories = filteredAutoFilledRuleConfigurations?.length > 0 ? filteredAutoFilledRuleConfigurations[0].ruleConfigurationCategories : []; 
    //array of objects each with input, output, operatorName, assumptionValue
   
    const currentCategoryRuleConfigurations = ruleConfigurationCategories?.length > 0 ? (ruleConfigurationCategories[formulaInternalKey - 1]?.ruleConfigurations || []) : []; 
...
const handleNext = () => {
   ...
   const visibleFormulas = currentRuleConfigurations.filter(item => 
       existingFormulaOutputs?.includes(item.output) && !deletedFormulas?.includes(item.output)
   );
...
useEffect(() => {
   const initialFormulas = filteredFormulas.map(item => ({
       ...
       showOnEstimationDashboard:false,
   }));

   const existingOutputs = new Set(formulaConfigValues.map(formula => formula.output));
   const newFormulas = initialFormulas?.filter(formula => 
       !existingOutputs.has(formula.output) &&
       !deletedFormulas.includes(formula.output)
   );
...
useEffect(() => {
    ...
    if(planObject?.assumptions?.length > 0 && !isLoadingPlanObject){
      const filteredAssumptions = planObject?.assumptions.filter(
         assumption => assumption.category === currentCategory
     );
...
return (
 ...
 if(isLoadingPlanObject){
   return <Loader/>
    ...
    return (
      <Fragment>
         <FormulaContext.Provider value={{formulaConfigValues, handleFormulaChange, setFormulaConfigValues, deletedFormulas, setDeletedFormulas, planObject}}>
