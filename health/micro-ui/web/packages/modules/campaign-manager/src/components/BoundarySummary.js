...
const BoundarySummary = (props) => {
  ...
  const campaignName = window.Digit.SessionStorage.get("HCM_CAMPAIGN_MANAGER_FORM_DATA")?.HCM_CAMPAIGN_NAME?.campaignName || t(LOCALIZATION.NA);
    ...
    {(isLoading || (!data && !error) || isFetching) && <Loader page={true} variant={"PageLoader"} loaderText={t(LOCALIZATION.DATA_SYNC_WITH_SERVER)}/>}
     <div className="container-full"> 
      ...
      <div style={{ display: "flex", justifyContent: "space-between" , marginTop: "1.5rem" }}>
        <HeaderComponent className="summary-header">{t(LOCALIZATION.ES_BOUNDARY_SUMMARY_HEADING)}</HeaderComponent>
      </div>
