...
const AppModule = () => {
  ...
  const handleNext = async () => {
    ...
    if (uniqueModules.length === 0) {
      setShowToast({ key: "error", label: t(LOCALIZATION.SELECT_ATLEAST_ONE_MODULE) });
      return;
    }
    ...
    if (!areAllowedModulesPresent) {
      setShowToast({
        key: "error",
        label: `${t(LOCALIZATION.HCM_MANDATORY_MODULES)} ${allowedModules?.allowedModule.map((m) => t(m)).join(", ")}`,
      });
      return;
    }
    ...
    for (const item of mdmsData || []) {
      ...
      try {
        ...
        setShowToast({ key: "error", label: t(LOCALIZATION.HCM_MDMS_DATA_UPDATE_ERROR) });
        return;
      } finally {
    ...
    for (const module of selectedModules) {
      ...
      for (const loc of locales) {
        ...
        try {
          ...
          setShowToast({ key: "error", label: t(LOCALIZATION.LOCALISATION_FETCH_ERROR) });
          return;
        } finally {
          setIsCreatingModule(false);
        }
        ...
        if (updatedLocalizations.length > 0) {
          try {
            ...
            setShowToast({ key: "error", label: t(LOCALIZATION.LOCALISATION_ERROR) });
            return;
          } finally {
            setIsCreatingModule(false);
          }
      ...
      //   setShowToast({ key: "error", label: t(LOCALIZATION.HCM_MDMS_DATA_UPSERT_ERROR) });
      //   return;
      // } finally {
      //   setIsCreatingModule(false);
      // }
  ...
  if (productTypeLoading || isLoading || mdmsData?.length == 0) {
    return <Loader page={true} variant={"OverlayLoader"} loaderText={t(LOCALIZATION.SAVING_FEATURES_CONFIG_IN_SERVER)} />;
  }
  ...
  return (
    <>
      <div>
        ...
        <TextBlock body="" caption={t(LOCALIZATION.CMP_DRAWER_WHAT_IS_MODULE_APP_CONFIG_SCREEN)} header="" captionClassName="camp-drawer-caption" subHeader="" />
      </div>
      {isCreatingModule && <Loader page={true} variant={"OverlayLoader"} loaderText={t(LOCALIZATION.COPYING_CONFIG_FOR_SELECTED_MODULES)} />}
            ...
            .map((module, index) => (
              <Card className={`module-card ${selectedModuleCodes.includes(module?.data?.name) ? "selected-card" : ""}`}>
                ...
                <Button
                  ...
                  label={selectedModuleCodes.includes(module?.data?.name) ? t(LOCALIZATION.DESELECT) : t(LOCALIZATION.ES_CAMPAIGN_SELECT)}
      ...
      <Footer
        actionFields={[
          <Button
            ...
            icon={"ArrowBack"}
          />,
          <Button label={t(LOCALIZATION.NEXT)} title={t(LOCALIZATION.NEXT)} variation="primary" icon={"ArrowDirection"} isSuffix onClick={handleNext} />,
        ]}
      />
      {showToast && (
        <Toast
