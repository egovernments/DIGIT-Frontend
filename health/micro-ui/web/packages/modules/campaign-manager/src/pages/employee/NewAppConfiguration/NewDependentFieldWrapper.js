...
function NewDependentFieldWrapper({ t }) {
    ...
    const RuleRow = ({ idx, onEdit, onDelete }) => (
        <div className="dependent-field-wrapper__rule-row">
            <Tag
                label={`${logicLabel} ${idx + 1}`}
                showIcon={false}
                stroke={true}
                className="dependent-field-wrapper__rule-tag version-tag"
                labelStyle={{ whiteSpace: "normal", wordBreak: "break-word" }}
            />

            <div className="dependent-field-wrapper__rule-actions">
                <div
                    role="button"
                    title={addDisplayLogicLabel}
                    aria-label={addDisplayLogicLabel}
                    onClick={() => onEdit(idx)}
                    className="dependent-field-wrapper__action-button"
                >
    ...
    return (
        <Card type="secondary">
            <div className="dependent-field-wrapper__header">
                <h3 className="dependent-field-wrapper__title">{displayLogicLabel}</h3>
            </div>

            <div className="dependent-field-wrapper__rules-list">
                {(!rules || rules.length === 0) ? (
                    <p className="dependent-field-wrapper__no-rules-text">{noLogicAddedLabel}</p>
                ) : (
                    <>
                        <RuleRow idx={0} onEdit={openEditor} onDelete={deleteRuleFromList} />
                        {rules.slice(1).map((r, i) => (
                            <React.Fragment key={`logic-block-${i + 1}`}>
                                <div className="dependent-field-wrapper__or-separator">
                                    <span className="dependent-field-wrapper__or-badge">
                                        {t("OR") || "OR"}
                                    </span>
                                </div>
            ...
            <div className="dependent-field-wrapper__add-button-container">
                <Button variation="secondary" label={addDisplayLogicLabel} title={addDisplayLogicLabel} onClick={openEditorForNew} icon="Add" className="dependent-field-wrapper__add-button" />
            </div>
                ...
                <BodyPortal>
                    <div className="popup-portal-overlay">
                        <PopUp
                            ...
                            children={[
                                <div key="single-rule-editor" className="dependent-field-wrapper__editor-content">
                                    <div className="dependent-field-wrapper__conditions-wrapper">
                                                    ...
                                                    <div className="dependent-field-popup__condition-card">
                                                        ...
                                                        <div className="dependent-field-popup__field-grid">
                                                            ...
                                                            <LabelFieldPair vertical removeMargin>
                                                                ...
                                                                <Dropdown
                                                                    ...
                                                                    selected={
                                                                        cond.leftPage
                                                                            ? pageOptions.find((p) => p.code === cond.leftPage) || {
                                                                                code: cond.leftPage,
                                                                ...
                                                                <Dropdown
                                                                    ...
                                                                    selected={
                                                                        cond.leftPage && cond.leftField
                                                                            ? leftFieldOptions.find((f) => f.code === cond.leftField) || { code: cond.leftField, label: cond.leftField }
                                                                            : { code: "", label: selectPageFirstLabel }
                                                                    }
                                                        ...
                                                        {!isProductVariantOrMultiselectField && (
                                                            <div className="dependent-field-popup__radio-cards">
                                                                {/* Enter Value Card */}
                                                                <div
                                                                    className={`dependent-field-popup__radio-card${!cond.isFieldComparison ? " dependent-field-popup__radio-card--selected" : ""}${!cond.leftField ? " dependent-field-popup__radio-card--disabled" : ""}`}
                                                                    onClick={() => {
                                                                        if (cond.leftField) {
                                                                            updateSubCond(idx, {
                                                                                isFieldComparison: false,
                                                                ...
                                                                >
                                                                    <div className="dependent-field-popup__radio-card-header">
                                                                        <div
                                                                            className={`dependent-field-popup__radio-circle${!cond.isFieldComparison ? " dependent-field-popup__radio-circle--selected" : ""}`}
                                                                        />
                                                                            ...
                                                                            <div className="dependent-field-popup__field-grid">
                                                                                <LabelFieldPair vertical removeMargin>
                                                                                    ...
                                                                                    <Dropdown
                                                                                        ...
                                                                                        selected={
                                                                                            cond.rightPage
                                                                                                ? pageOptions.find((p) => p.code === cond.rightPage) || { code: cond.rightPage, displayName: cond.rightPage }
                                                                                                : {}
                                                                                        }
                                                                                        disabled={!cond.leftField}
                                                                                    />
                                                                                    ...
                                                                                    <Dropdown
                                                                                        ...
                                                                                        selected={
                                                                                            cond.rightPage && cond.rightField
                                                                                                ? rightFieldOptions.find((f) => f.code === cond.rightField) || { code: cond.rightField, label: cond.rightField }
                                                                                                : { code: "", label: selectPageFirstLabel }
                                                                                        }
                                        ...
                                        <div className="dependent-field-wrapper__add-condition-wrapper">
                                            <Button variation="secondary" icon={"Add"} size={"medium"} label={addConditionLabel} title={addConditionLabel} onClick={addSubCondition} />
                                        </div>
                                        ...
                                        {generateConditionSummary(draftRule?.conds) && (
                                            <div className="rule-summary">
                                                ...
                                                <div className="rule-summary__then-show">
                                                    ...
                                                    <Tag
                                                        label={useT(selectedField?.label) || useT(selectedField?.fieldName) || "Field"}
                                                        type="monochrome"
