...
const ColumnTemplate = ({ field, t, fieldTypeMasterData, selectedField, onFieldClick, data }) => {
  ...
  const validChildren = children.filter((child, index) => {
    ...
    if (!allowed) {
      console.warn(
        `Child at index ${index} with format "${child?.format}" is not allowed in Column. Allowed: [${allowedChildren.join(", ")}]`
      );
    }
    return allowed;
  });
  ...
  return (
    ...
    >
      {children.map((child, index) => {
        ...
        return renderTemplateComponent(childWithId, fieldTypeMasterData, selectedField, t, onFieldClick, data, `column-${field.id}`, index);
      })}
    </div>
  );
};
