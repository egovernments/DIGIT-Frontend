...
const RowTemplate = ({ field, t, fieldTypeMasterData, selectedField, onFieldClick, data }) => {
  ...
  console.log("RowTemplate rendering:", { 
  ...
  const validChildren = children.filter((child, index) => {
    ...
    if (!allowed) {
      console.warn(`Child at index ${index} with format "${child?.format}" is not allowed in Row. Allowed: [${allowedChildren.join(", ")}]`);
    }
    return allowed;
  });
  ...
  return (
    <div style={{
      ...
      alignItems: properties.crossAxisAlignment === "start" ? "flex-start" :
                  ...
                  properties.crossAxisAlignment === "stretch" ? "stretch" : "center",
