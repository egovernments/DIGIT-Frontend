...
const LocalisationBulkUpload = () => {
  ...
  const { data: localeData = [] } = Digit.Hooks.useCustomMDMS(stateId, "common-masters", [{ name: "StateInfo" }], {
    select: (data) => {
      ...
      const defaultLanguage = { label: t(LOCALIZATION.DIGIT_DEFAULT_MESSAGE), value: "default" };
  ...
  // Module options
  const regexOption = [
    { code: t(LOCALIZATION.DIGIT_HCM_INVENTORY_MODULE), value: `hcm-inventory-${campaignNumber}` },
    { code: t(LOCALIZATION.DIGIT_HCM_REGISTRATION_MODULE), value: `hcm-registration-${campaignNumber}` },
    { code: t(LOCALIZATION.DIGIT_HCM_DELIVERY_MODULE), value: `hcm-delivery-${campaignNumber}` },
    { code: t(LOCALIZATION.DIGIT_HCM_HFREFERRAL_MODULE), value: `hcm-hfreferral-${campaignNumber}` },
    { code: t(LOCALIZATION.DIGIT_HCM_COMPLAINTS_MODULE), value: `hcm-complaints-${campaignNumber}` },
  ];
  ...
  useEffect(() => {
    const fetchLocalizations = async () => {
      ...
      try {
        ...
        console.error(err);
        setShowToast({
          label: t(LOCALIZATION.DIGIT_LOC_MODULE_DATA_LOAD_FAILED),
          type: "error",
        });
      } finally {
  ...
  const onBulkUploadModalSubmit = async (file) => {
    try {
      if (!file) {
        setShowToast({ label: t(LOCALIZATION.DIGIT_LOC_INVALID_FILE), type: "error" });
        return;
      }
      if (!choosenModule?.value) {
        setShowToast({ label: t(LOCALIZATION.DIGIT_LOC_SELECT_MODULE_FIRST), type: "error" });
        return;
      }

      const parseFn = Digit?.Utils?.parsingUtils?.parseXlsToJsonMultipleSheetsFile;
      if (!parseFn) {
        setShowToast({ label: t(LOCALIZATION.DIGIT_LOC_PARSER_NOT_AVAILABLE), type: "error" });
        return;
      }
      ...
      if (!updatedResult?.length) {
        setShowToast({ label: t(LOCALIZATION.DIGIT_LOC_EMPTY_OR_INVALID_FILE), type: "error" });
        return;
      }
      ...
      if (payload.length === 0) {
        setShowToast({ label: t(LOCALIZATION.DIGIT_LOC_NO_VALID_ENTRIES), type: "error" });
        return;
      }
      ...
      setShowToast({ label: t(LOCALIZATION.DIGIT_LOC_UPSERT_SUCCESS) });
      setShowBulkUploadModal(false);
    } catch (error) {
      console.error("❌ Upload error:", error);
      const msg = error?.response?.data?.Errors?.[0]?.message || error?.message || t(LOCALIZATION.DIGIT_LOC_UPLOAD_UNKNOWN_ERROR);
  ...
  return (
    <React.Fragment>
      <Header>{t(LOCALIZATION.DIGIT_LOC_BULK_UPLOAD_XLS)}</Header>

      <Card>
        {/* Module selector */}
        <div style={{ marginBottom: "1rem" }}>
          <h3 style={{ marginBottom: "0.5rem" }}>{t(LOCALIZATION.DIGIT_LOC_SELECT_MODULE_LABEL)}</h3>
          <Dropdown
            style={{ width: "60%" }}
            t={t}
            option={regexOption}
            ...
            placeholder={t(LOCALIZATION.DIGIT_LOC_SELECT_MODULE_PLACEHOLDER)}
          />
        ...
        <div style={{ display: "flex", gap: "1rem" }}>
          <Button
            variation="secondary"
            label={t(LOCALIZATION.DIGIT_LOC_DOWNLOAD_TEMPLATE)}
          ...
          <Button
            variation="primary"
            label={t(LOCALIZATION.DIGIT_LOC_BULK_UPLOAD_BUTTON)}
      ...
      {showBulkUploadModal && (
        <FileUploadModal
          heading={t(LOCALIZATION.DIGIT_LOC_BULK_UPLOAD_HEADER)}
          cancelLabel={t(LOCALIZATION.DIGIT_LOC_MODAL_CANCEL)}
          submitLabel={t(LOCALIZATION.DIGIT_LOC_MODAL_SUBMIT)}
