const getTypeAndMetaData = (field, fieldTypeMasterData = []) => {
  ...
  // Special handling for document sections
  if (field.type === "documentSection") {
    return {
      type: "documentSection",
      appType: "documentSection",
      allowedFieldTypes: ["documentUpload", "documentUploadAndDownload"],
      maxFiles: field.maxFiles || 5,
      allowedFileTypes: field.allowedFileTypes || ["pdf", "jpg", "jpeg", "png", "doc", "docx"],
...
export const restructure = (data1, fieldTypeMasterData = [], parent, t) => {
  return [...data1?.sort((a, b) => a.order - b.order)].map((page) => {
    const cardFields = page.properties
      ...
      ?.map((field, index) => {
        const fieldData = getTypeAndMetaData(field, fieldTypeMasterData);
        
        // Special handling for document sections
        if (fieldData.type === "documentSection") {
          return {
            ...fieldData,
            ...flattenValidationsToField(field?.validations || []),
            label: field?.label || t("DOCUMENT_SECTION"),
            value: field?.value || "",
            defaultValue: field?.value ? true : false,
            active: true,
            jsonPath: field?.fieldName || `documentSection_${index}`,
            format: field?.format || "documentSection",
            metaData: {
              allowedFieldTypes: fieldData.allowedFieldTypes,
              maxFiles: fieldData.maxFiles,
              allowedFileTypes: fieldData.allowedFileTypes,
              maxFileSize: fieldData.maxFileSize,
            },
            // Document sections are always required and cannot be hidden
            hidden: false,
            deleteFlag: false,
            isLocalised: field?.isLocalised ? true : false,
            innerLabel: field?.innerLabel || t("DOCUMENT_SECTION_DESCRIPTION"),
            errorMessage: field?.errorMessage || "",
            tooltip: field?.tooltip || t("DOCUMENT_SECTION_TOOLTIP"),
            infoText: field?.infoText || t("DOCUMENT_SECTION_INFO"),
            order: field?.order,
            readOnly: false, // Document sections cannot be read-only
            systemDate: false,
            pattern: field?.validations?.find((i) => i?.type === "pattern"),
            RegexPattern: field?.validations?.find((i) => i?.type === "pattern") ? true : false,
            MdmsDropdown: false, // Document sections don't use MDMS
            isMdms: false,
            isMultiSelect: false,
            includeInForm: true, // Document sections are always included in form
            includeInSummary: field?.includeInSummary !== false, // Default to true
            helpText: typeof field?.helpText === "string" ? field.helpText : t("DOCUMENT_SECTION_HELP_TEXT"),
          };
        }
          ...
          headerFields: [
            {
              ...
              metaData: {},
              required: true,
              isLocalised: page.label && page.label.includes("_") ? true : false,
            },
            {
              type: "textarea",
              label: "SCREEN_DESCRIPTION",
              value: page.description && !page.description.includes("_") ? page.description : "Description for Section 1",
              active: true,
              jsonPath: "Description",
              metaData: {},
              required: true,
              isLocalised: page.description && page.description.includes("_") ? true : false,
            },
          ],
        },
      ],
  ...
  return updatedData.map((section, index) => {
    const properties = section.cards?.[0]?.fields.map((field, fieldIndex) => {
      ...
      if (field.type === "documentSection") {
        return {
          type: "documentSection",
          label: field?.label || "Document Section",
          order: fieldIndex + 1,
          value: field?.value || "",
          hidden: false,
          fieldName: field?.jsonPath || `documentSection_${fieldIndex}`,
          tooltip: field?.tooltip || "",
          infoText: field?.infoText || "",
          innerLabel: field?.innerLabel || "",
          errorMessage: field?.errorMessage || "",
          ...
          // Document section specific properties
          maxFiles: field?.metaData?.maxFiles || 5,
          allowedFileTypes: field?.metaData?.allowedFileTypes || ["pdf", "jpg", "jpeg", "png", "doc", "docx"],
          maxFileSize: field?.metaData?.maxFileSize || 10,
          allowedFieldTypes: field?.metaData?.allowedFieldTypes || ["documentUpload", "documentUploadAndDownload"],
        };
      }
...
export const normalizeAllowedFileTypes = (allowedFileTypes) => {
  if (!allowedFileTypes) {
    return ["pdf", "doc", "docx", "jpeg", "jpg", "png"];
  }
  
  if (Array.isArray(allowedFileTypes)) {
    return allowedFileTypes.filter(type => type && type.trim().length > 0);
  }
