...
export const RenderConditionalField = ({
  ...
  switch (cField?.type) {
    ...
    case "date":
    case "time":
      const fieldLabel = cField?.label || cField?.bindTo || "";
      return (
        <FieldV1
          type={cField?.type}
          label={cField?.label}
          ...
          value={
            // Comment out localization logic - use plain text values directly
            // isLocalisable ? useCustomT(drawerState?.[cField?.bindTo]) : drawerState?.[cField?.bindTo] === true ? "" : drawerState?.[cField?.bindTo]
            drawerState?.[cField?.bindTo] === true ? "" : drawerState?.[cField?.bindTo] || ""
          }
          config={{
            step: "",
          }}
          onChange={(event) => {
            ...
            // Comment out localization logic - store plain text values directly
            // if (isLocalisable) {
            //   updateLocalization(
          ...
          infoMessage={t("INFO_" + fieldLabel.toUpperCase().split(" ").join("_"))}
          disabled={disabled}
        />
      );
    case "options":
      return (
        <div
          style={{ padding: "1rem", border: "1px solid #c84c0e", borderRadius: "4px", display: "flex", flexDirection: "column", gap: "1.5rem" }}
        >
                  ...
                  setDrawerState((prev) => ({
                    ...
                    [cField?.bindTo]: prev?.[cField?.bindTo]?.map((i) => {
                      if (i.code && i.code === item.code) {
                        ...
                        return {
                          ...i,
                          name: event.target.value, // Store plain text value directly
                        };
                      }
                      return i;
                    }),
