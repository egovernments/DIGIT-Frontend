...
const FetchFromMicroplanScreen = () => {
    ...
    const fetchTemplates = async () => {
      if (data && data?.updatedCampaignData) {
        ...
        try {
          ...
          setShowToast({ key: "warning", label: t(I18N_KEYS.COMPONENTS.EMPTY_TEMPLATE_GENERTAION_NOTCOMPLETED_STILL_PROCEEDING) });
          setCurrentStep((prev) => TEMPLATE_GENERATION_STEP);
          setTemplates({
            facilityFile: null,
            targetFile: null,
    ...
    const fetchMicroplanData = async () => {
      try {
        ...
        setTimeout(() => {
          ...
          setShowToast({ key: "info", label: t(I18N_KEYS.COMPONENTS.FETCHING_MICROPLAN_DATA) });
        }, MICROPLAN_FETCH_TIMEOUT);
      } catch (error) {
        console.error("Error fetching microplan data:", error);
        setShowToast({ key: "error", label: t(I18N_KEYS.COMPONENTS.ERROR_FETCHING_MICROPLAN) });
      }
    };
  ...
  useEffect(() => {
    const navigateTimeout = setTimeout(() => {
      setShowToast({ key: "info", label: t(I18N_KEYS.COMPONENTS.PLS_WAIT_UNTIL_PAGE_REDIRECTS) });
    }, 3000);
    return () => clearTimeout(navigateTimeout); // Cleanup timeout
  }, []);
  ...
  useEffect(() => {
    if (showToast?.key != "error" && error) {
      setShowToast({ key: "error", label: t(I18N_KEYS.COMPONENTS.SOME_ERROR_OCCURED_IN_FETCH) });
    }
  }, [error]);
      ...
      if (currentStep === MICROPLAN_STOP_STEP && completed == null) {
        try {
          ...
          if (response?.resources && response?.resources?.length == 3) {
            ...
            setShowToast({ key: "warning", label: t(I18N_KEYS.COMPONENTS.SOME_ERROR_OCCURED_IN_FETCH_RETRYING) });
          ...
          setShowToast({ key: "error", label: t(I18N_KEYS.COMPONENTS.ERROR_FETCHING_CAMPAIGN) });
        }
      }
    };
    fetchCampaign();
  ...
  useEffect(() => {
    async function useeffectfetchfrommicrploan() {
      if (currentStep === steps.length && microplan) {
        setShowToast({ key: "success", label: t(I18N_KEYS.COMPONENTS.CMN_ALL_DATA_FETCH_DONE) });
  ...
  return (
    <span className="fetch-from-microplan">
      <HeaderComponent>{t(I18N_KEYS.COMPONENTS.MY_FETCH_FROM_MICROPLAN_HEADING)}</HeaderComponent>
      <AlertCard
        populators={{
          name: "infocard",
        }}
        variant="info"
        text={t(I18N_KEYS.COMPONENTS.HCM_FETCH_FROM_PLAN_INFO)}
      />
        ...
        <ul className="sandbox-installation-steps">
          ...
          <li key={currentStep + 1} className={`sandbox-step ${0 < currentStep ? "sandbox-visible" : ""}`}>
            <span className="sandbox-step-text">
              {t(I18N_KEYS.COMPONENTS.MP_COMPELTED_STEPS)} - {currentStep} {t(I18N_KEYS.COMPONENTS.MP_OUT_OFF)} - {steps?.length}
            </span>
          </li>
        </ul>
      </div>
