...
const toDateInput = (epochOrString) => {
  if (!epochOrString) return "";
  const d = new Date(Number.isFinite(Number(epochOrString)) ? Number(epochOrString) : epochOrString);
...
const DeliveryStrategySetup = ({ onSelect, formData, control, ...props }) => {
  ...
  const [fromDate, setFromDate] = useState(toDateInput(savedCycle?.fromDate));
  const [toDate, setToDate] = useState(toDateInput(savedCycle?.toDate));
  ...
  useEffect(() => {
    ...
    if (cycle?.fromDate) setFromDate(toDateInput(cycle.fromDate));
    if (cycle?.toDate) setToDate(toDateInput(cycle.toDate));
  ...
  const campaignRange = useMemo(
    () => ({
      startDate: formStorageData?.HCM_CAMPAIGN_DATE?.campaignDates?.startDate || toDateInput(campaignData?.startDate),
      endDate: formStorageData?.HCM_CAMPAIGN_DATE?.campaignDates?.endDate || toDateInput(campaignData?.endDate),
    }),
            ...
            <LabelFieldPair>
              ...
              <div className="date-field-container" style={{padding:"0rem"}}>
                <FieldV1
                  ...
                  onChange={(d) => {
                    setFromDate(d);
                    if (toDate && d && toDate < d) setToDate("");
                  }}
                />
                <FieldV1
                  type="date"
                  ...
                  withoutLabel={true}
                  disabled={!fromDate}
                  populators={{ newDateFormat: true, min: fromDate || campaignRange?.startDate, max: campaignRange?.endDate }}
                  onChange={(d) => {
                    setToDate(d);
                  }}
                />
