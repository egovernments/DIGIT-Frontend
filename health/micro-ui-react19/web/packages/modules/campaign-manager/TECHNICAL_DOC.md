...
| Date range selection issue fixed in Commodity Management; "Rejected" column removed from Transaction Summary; sheet issue fixed in upcoming campaign edit flow; custom reports status check and end date update | [#4244](https://github.com/egovernments/DIGIT-Frontend/pull/4244) |
...
| Localization search call fixed (unnecessary locale modules removed from the call); ship commodity button placement corrected; loader centering fixed; unused locale keys cleaned up | [#4263](https://github.com/egovernments/DIGIT-Frontend/pull/4263) |
...
| Boundary column labels in `SelectingBoundariesDuplicate.js` now show translated names using the hierarchy's localization module; core library version upgraded | [#4281](https://github.com/egovernments/DIGIT-Frontend/pull/4281) |
