...
export const UICustomizations = {
  AssignCampaignInboxConfig: {
    preProcess: (data) => {
      ...
      // Never read from body.Projects - InboxSearchComposer spreads that path into a
      // plain object ({ "0": {...} }), which project search rejects with JsonMappingException.
      const allProjects = Object.values(body?.jurisdictionProjects || {});
      ...
      // and mutates Projects in place afterwards, so assigning onto data.body lets the
    ...
    additionalCustomizations: (row, key, column, value, t, searchResult) => {
      ...
      switch (key) {
        ...
        case "PROJECT_BOUNDARY_TYPE": {
          ...
          if (!value) return <span>{t(I18N_KEYS.COMMON.CORE_COMMON_NA)}</span>;
          return <span>{hierarchy ? t(`${hierarchy}_${value}`) : t(value)}</span>;
        }

        case "PROJECT_BOUNDARY":
        case "PROJECT_TYPE":
          return value ? <span>{t(`${value}`)}</span> : <span>{t(I18N_KEYS.COMMON.CORE_COMMON_NA)}</span>;
        ...
        default:
          return t(I18N_KEYS.COMMON.CORE_COMMON_NA);
      }
    },
  },
