...
import { Card, Button, Header } from "@egovernments/digit-ui-react-components";
...
const Jurisdictions = ({ config, onSelect, formData }) => {
  ...
  const handleAdd = () => setBoundaryList([...boundaryList, { id: Date.now() }]);
  ...
  const isLastBoundaryValid = () => {
    ...
    return lastValue?.boundaryType && lastValue?.boundary;
  };
    ...
    <div className="boundary-list-wrapper">
      {selectedHierarchy &&
        boundaryList.map((entry, index) => (
          <Card
            key={entry.id}
            style={{ marginBottom: "16px", position: "relative", backgroundColor: "#EEEEEE", border: "1px solid rgb(0,0,0)", padding: "16px" }}
          >
      ...
      {selectedHierarchy && (
        <div>
          <Button
            label={t("ADD_JURISDICTION")}
            onButtonClick={handleAdd}
            disabled={boundaryList.length > 0 && !isLastBoundaryValid()}
          />
        </div>
      )}
    </div>
