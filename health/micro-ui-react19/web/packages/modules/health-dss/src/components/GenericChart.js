...
const GenericChart = ({
  ...
  function download(data) {
    ...
    setTimeout(() => {
      switch (data.code) {
        case "pdf":
          return PDFDownload(chart, t(header));
        case "image":
          return ChartImageDownload(chart, t(header));
        case "sharePdf":
          return SharePDF(tenantId, chart, t(header), data.target);
        case "shareImage":
          return ShareChartImage(tenantId, chart, t(header), data.target);
        default:
          return null;
      }
    }, 1000);
