...
import { Toast, Loader } from "@egovernments/digit-ui-components";
...
const IFrameInterface = (props) => {
  ...
  useEffect(() => {
    ...
    url = Digit?.Customizations?.["commonUiConfig"]?.["IframeInterfaceConfig"]?.addAdditionalFilters
      ? Digit?.Customizations?.["commonUiConfig"]?.["IframeInterfaceConfig"]?.addAdditionalFilters(url, filters?.filters)
      : url;
  ...
  return (
    <React.Fragment key={location.pathname}>
      ...
      <div className={`app-iframe-wrapper ${props?.wrapperClassName ? props?.wrapperClassName : ""}`}>
        ...
        />
        {iframeLoaded && <Toast type={iframeLoaded ? "info" : ""} label={"Iframe Loaded"} />}
      </div>
    </React.Fragment>
  );
};
