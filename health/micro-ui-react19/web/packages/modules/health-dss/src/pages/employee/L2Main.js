...
import { DownloadIcon, FilterIcon, ShareIcon } from "@egovernments/digit-ui-components";
...
const getInitialRange = (location) => {
...
const L2Main = ({}) => {
  ...
  const handlePrint = () => PDFDownload(fullPageRef, t(dashboardConfig?.[0]?.name));
  ...
  const onActionSelect = (item) => {
    switch (item?.code) {
      case "ES_DSS_DOWNLOAD_IMAGE":
        setTimeout(() => {
          return ImageDownload(fullPageRef, t(dashboardConfig?.[0]?.name));
        }, 500);
        break;
      ...
      case "ES_DSS_SHARE_PDF_EMAIL":
        setTimeout(() => {
          return SharePDF(tenantId, fullPageRef, t(dashboardConfig?.[0]?.name), "mail");
        }, 500);
        break;
      case "ES_DSS_SHARE_PDF_WHATSAPP":
        setTimeout(() => {
          return SharePDF(tenantId, fullPageRef, t(dashboardConfig?.[0]?.name), "whatsapp");
        }, 500);
        break;
      case "ES_DSS_SHARE_IMAGE_EMAIL":
        setTimeout(() => {
          return ShareImage(tenantId, fullPageRef, t(dashboardConfig?.[0]?.name), "mail");
        }, 500);
        break;
      case "ES_DSS_SHARE_IMAGE_WHATSAPP":
        setTimeout(() => {
          return ShareImage(tenantId, fullPageRef, t(dashboardConfig?.[0]?.name), "whatsapp");
        }, 500);
        break;
      case "ES_DSS_SHARE_PDF":
        setTimeout(() => {
          return ShareImage(tenantId, fullPageRef, t(dashboardConfig?.[0]?.name));
        }, 500);
        break;
