...
import useCustomAPIHook from "../../../../../../ui-libraries/src/hooks/useCustomAPIHook.js";
...
const InboxSearchComposer = ({
  ...
  const mobileSearchSession = Digit.Hooks.useSessionStorage(
    "MOBILE_SEARCH_MODAL_FORM",
    {}
  );
  ...
  useEffect(() => {
    ...
    if (Object.keys(state.searchForm)?.length >= 0) {
      const result = {
        ..._.get(apiDetails, apiDetails?.searchFormJsonPath, {}),
        ...state.searchForm,
      };
    ...
    if (Object.keys(state.filterForm)?.length >= 0) {
      const result = {
        ..._.get(apiDetails, apiDetails?.filterFormJsonPath, {}),
        ...state.filterForm,
      };
      Object.keys(result).forEach((key) => {
    ...
    if (Object.keys(state.tableForm)?.length >= 0) {
      _.set(apiDetails, apiDetails?.tableFormJsonPath, {
        ..._.get(apiDetails, apiDetails?.tableFormJsonPath, {}),
        ...state.tableForm,
      });
    }
    const searchFormParamCount = Object.keys(state.searchForm).reduce(
      (count, key) => (state.searchForm[key] === "" ? count : count + 1),
      0
    );
    const filterFormParamCount = Object.keys(state.filterForm).reduce(
      (count, key) => (state.filterForm[key] === "" ? count : count + 1),
      0
    );

    if (
      Object.keys(state.tableForm)?.length > 0 &&
      (searchFormParamCount >= apiDetails?.minParametersForSearchForm ||
        filterFormParamCount >= apiDetails?.minParametersForFilterForm)
    ) {
      setEnable(true);
    }
  ...
  const updatedReqCriteria = Digit?.Customizations?.[apiDetails?.masterName]?.[
    apiDetails?.moduleName
  ]?.preProcess
    ? Digit?.Customizations?.[apiDetails?.masterName]?.[
        apiDetails?.moduleName
      ]?.preProcess(requestCriteria, configs.additionalDetails)
    : requestCriteria;

  if (configs.customHookName) {
    var { isLoading, data, revalidate, isFetching, refetch, error } = eval(
      `Digit.Hooks.${configs.customHookName}(updatedReqCriteria)`
    );
  } else {
    var { isLoading, data, revalidate, isFetching, error } =
  ...
  return (
    <InboxContext.Provider value={{ state, dispatch }}>
      {headerLabel && (
        <Header className="digit-form-composer-header">{t(headerLabel)}</Header>
      )}
