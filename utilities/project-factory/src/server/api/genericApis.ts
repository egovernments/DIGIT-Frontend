...
async function createAndUploadFile(updatedWorkbook: XLSX.WorkBook, request: any, tenantId?: any) {
    ...
    if (!responseData) {
        throwError("COMMON", 500, "INTERNAL_SERVER_ERROR", "Error while uploading excel file")
    }
...
async function getAutoGeneratedBoundaryCodesHandler(boundaryList: any, childParentMap: Map<{ key: string; value: string; }, { key: string; value: string; } | null>, elementCodesMap: any, countMap: any, request: any) {
    try {
        ...
        console.log(boundaryList, "1111111111")
        console.log(childParentMap, "2222222222222222")
...
async function getAutoGeneratedBoundaryCodes(boundaryList: any, childParentMap: any, elementCodesMap: any, countMap: any, request: any) {
    ...
    console.log(boundaryList, "listttttttttttttttt")
    console.log(childParentMap, "cccccccccccccccccc")
    ...
    console.log(columnsData, "columnnnnnnnnnnnnnnnnn")

    // // Create a set to store unique elements
    // const elementSet = new Set<any>();
    // columnsData.forEach(column => {
    //     column.forEach(element => {
    //         elementSet.add(element);
    //     });
    // });
    // console.log(elementSet, "Settttttttttttttttt")
    ...
    for (let i = 0; i < columnsData.length; i++) {
        ...
        for (const element of column) {
            if (!elementCodesMap.has(element)) {
                console.log(element, "qqqqqqqqqqqqqqq")
                console.log(childParentMap, "mmmmmmmmmmmm")
                const parentCode = findMapValue(childParentMap,element)
                console.log(parentCode, "parentttttttttttttttttt")
                if (parentCode !== undefined && parentCode !== null ) {
                    ...
                    if (grandParentCode != null && grandParentCode != undefined) {
                        ...
                        console.log(element.value,"wwwwwwww")
                       console.log(findMapValue(elementCodesMap,parentCode),"9999999999999999")
                    ...
                    console.log(element.value,"iiiiiiiiiiiiiiiiii")
    ...
    console.log(countMap, "opeeeeeeeeeerrrrrrrrrrr")
    console.log(elementCodesMap, "elllllllllllllllllllllll")
        ...
        Array.from(boundaryMap.entries()).forEach(async ([boundaryName, boundaryCode]) => {
            if (!codeSet.has(boundaryCode)) {
                const boundary = {
                    ...
                    geometry: null,
                    additionalDetails: {
                        name: boundaryName.value
                    }
                };
                console.log(boundary,"boundaryentiyyyyyyyyyyyyyyyyyyyyy")
 ...
 */
async function createBoundaryRelationship(request: any, boundaryTypeMap: Map<{ key: string; value: string; }, string>, modifiedChildParentMap: any, localizationMap?: any) {
    try {
        ...
        let flag = 1;
        const boundaryTypeObj = Object.fromEntries(Array.from(boundaryTypeMap.entries()));
        for (const [boundaryCode, boundaryType] of Object.entries(boundaryTypeObj)) {
