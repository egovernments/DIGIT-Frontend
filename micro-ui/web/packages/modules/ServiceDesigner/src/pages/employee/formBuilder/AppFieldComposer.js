...
// Component to toggle visibility of a field for default (non-deletable) fields
const ToggleVisibilityControl = ({ config, onToggle }) => {
  const isVisible = config?.hidden !== true;
  ...
  return (
    ...
    >
      <Switch
        key={`toggle-${config?.jsonPath}-${isVisible}`}
        label=""
        isCheckedInitially={isVisible}
      />
    </div>
  );
};
...
const PanelFieldDisplay = ({ t, label, appType, config, onToggle, onDelete, isMandatory, isApplicantMandatory }) => {
  const addressDefaultFieldPaths = ["AddressPincode", "AddressStreet", "AddressLocation", "AddressMapCoord"];
  const isAddressDefaultField = addressDefaultFieldPaths.includes(config?.jsonPath);

  const isNewlyAddedField = config?.deleteFlag === true && !isAddressDefaultField;
  const isDefaultField = config?.deleteFlag === false || isAddressDefaultField || config?.deleteFlag === undefined;
  const isMandatoryField = isMandatory || isApplicantMandatory;

  const getFieldTypeLabel = () => {
    if (appType && appType.trim() !== "") {
      return appType;
    }
    return config?.appType || config?.type || "Field";
  };
  ...
  return (
    <>
      <div className="appConfigLabelField-label-container">
        ...
        <Tag icon="" label={getFieldTypeLabel()} className="app-config-field-tag" labelStyle={{}} showIcon={false} style={{}} />
      </div>

      {/* Default fields (not mandatory) - show toggle to hide/show */}
      {isDefaultField && !isMandatoryField && !isNewlyAddedField && (
        <ToggleVisibilityControl config={config} onToggle={onToggle} />
      )}

      {/* Newly added fields - show delete icon */}
      {isNewlyAddedField && !isMandatoryField && (
        <DeleteFieldControl isDelete={true} onDelete={onDelete} />
      )}
    </>
  );
};
            ...
            {!headerFields && isDrawer ? (
              <>
                <PanelFieldDisplay
                  ...
                  isApplicantMandatory={isApplicantMandatory}
                />
              </>
            ) : (
              <>
...
function AppFieldComposer({
  ...
  const isApplicantMandatory = isApplicantSection && (label === "Name" || label === "Mobile Number");
