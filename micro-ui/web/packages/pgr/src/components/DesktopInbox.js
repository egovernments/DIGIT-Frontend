...
import { Card, Loader } from "@digit-ui/digit-ui-react-components";
    ...
    () => [
      {
        ...
        Cell: ({ row }) => {
          return (
            <div>
              <span className="link">
                <Link to={`/${window?.contextPath}/employee/pgr/complaint/details/` + row.original["serviceRequestId"]}>{row.original["serviceRequestId"]}</Link>
              </span>
