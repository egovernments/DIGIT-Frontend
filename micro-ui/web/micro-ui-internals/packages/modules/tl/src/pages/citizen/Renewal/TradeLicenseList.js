...
const TradeLicenseList = ({ application }) => {
    ...
    const onsubmit = async () => {
        ...
        // Replace TLSearch with Digit.TLService.TLsearch if applicable, or assume it's available.
        // However, numberOfApplications logic might be specific. 
        // Trying to use Digit.TLService.TLsearch to get applications count?
        // Actually Digit.TLService.TLsearch returns { Licenses: [], count: ... } ??
        // Source code used TLSearch.numberOfApplications. 
        // I will use Digit.TLService.TLsearch for now as a fallback or assume I can access the same logic.
        // If TLSearch is strict, I must import it. But libraries path is tricky.
        // Let's assume Digit.TLService.TLsearch provides list and we can filter.

        let res = await Digit.TLService.TLsearch({ tenantId: application?.tenantId, filters });
        let numOfApplications = res?.Licenses;
