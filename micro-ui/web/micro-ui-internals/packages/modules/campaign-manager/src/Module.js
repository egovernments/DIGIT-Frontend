...
if (typeof window !== "undefined") {
  window.addEventListener("error", (event) => {
    console.error("❌ Global error caught in Campaign:", event.error || event.message, event);
  });

  window.addEventListener("unhandledrejection", (event) => {
    console.error("❌ Unhandled Promise Rejection in Campaign:", event.reason || event);
  });
}

console.log("✅ Campaign Module.js loaded before export");
...
const initCampaignComponents = () => {
  if (!window?.Digit?.Hooks) {
    console.warn("Digit not ready yet, skipping Campaign init");
    return;
  }
  overrideHooks();
  updateCustomConfigs();
