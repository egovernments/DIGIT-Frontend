        ...
        Digit.WSService.wsCalculationEstimate(data, businessService)
          ...
          .catch((e) => {
            setShowToast({
              ...
              isWarning: false,
              key: "error",
              message: e?.response?.data?.Errors[0]?.message ? t(`${e?.response?.data?.Errors[0]?.code}`) : t("PT_COMMON_ADD_REBATE_PENALITY"),
            });
          });
      }
    } else {
          ...
          <StatusTable>
              ...
              {values?.map((value, index) => {
                return (
                  <Row
                    ...
                    key={`${value.title}`}
                    label={`${t(`${value.title}`)}`}
                    text={value?.value ? Number(value?.value).toFixed(2) : ""}
                  />
                );
              })}
            </div>
            ...
            <div>
              <Row
                ...
                text={Number(billDetails?.totalAmount).toFixed(2) || 0}
              />
              <Row
                className="border-none"
