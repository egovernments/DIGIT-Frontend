...
const PTDocumentUploadAssessmentForm = ({ onSelect, config, formData, errors }) => {
    ...
    const savedData = formData?.documents ? formData : ({});
    ...
    const registrationProofDoc = ownerDocuments.find((doc) => doc.code === "OWNER.REGISTRATIONPROOF");
    const usageProofDoc = ownerDocuments.find((doc) => doc.code === "OWNER.USAGEPROOF");
    const specialCategoryProofDoc = ownerDocuments.find((doc) => doc.code === "OWNER.SPECIALCATEGORYPROOF");
    ...
    const registrationProofDocuments = registrationProofDoc?.hasDropdown && registrationProofDoc?.dropdownData
        ? registrationProofDoc.dropdownData
            .filter((item) => item.active)
            .map((item) => ({
                code: item.code,
                name: t(item.code.replace(/\./g, "_")),
    ...
    const usageProofDocuments = usageProofDoc?.hasDropdown && usageProofDoc?.dropdownData
        ? usageProofDoc.dropdownData
            .filter((item) => item.active)
            .map((item) => ({
                code: item.code,
                name: t(item.code.replace(/\./g, "_")),
    ...
    const specialCategoryProofDocuments = specialCategoryProofDoc?.hasDropdown && specialCategoryProofDoc?.dropdownData
        ? specialCategoryProofDoc.dropdownData
            .filter((item) => item.active)
            .map((item) => ({
                code: item.code,
                name: t(item.code.replace(/\./g, "_")),
    ...
    React.useEffect(() => {
        if (ownerDocuments.length > 0 && documents.length === 0) {
            ...
            if (registrationProofDoc) {
                initialDocs.push({
                    documentType: "OWNER.REGISTRATIONPROOF",
                    documentCode: "",
                    file: null,
                    fileStoreId: null,
                    fileName: "",
                    required: registrationProofDoc.required || false,
                });
            }
            if (usageProofDoc) {
                initialDocs.push({
                    documentType: "OWNER.USAGEPROOF",
                    documentCode: "",
                    file: null,
                    fileStoreId: null,
                    fileName: "",
                    required: usageProofDoc.required || false,
                });
            }
            if (specialCategoryProofDoc) {
                initialDocs.push({
                    documentType: "OWNER.SPECIALCATEGORYPROOF",
                    documentCode: "",
                    file: null,
                    fileStoreId: null,
                    fileName: "",
                    required: specialCategoryProofDoc.required || false,
                });
            }
    ...
    const handleRegistrationProofUpload = async (fileData) => {
        const uploadErrors = [];
        for (const file of fileData) {
            if (file.size >= 5000000) {
                uploadErrors.push({ file, error: t("PT_FILE_SIZE_EXCEEDS_5MB") });
    ...
    const handleUsageProofUpload = async (fileData) => {
        const uploadErrors = [];
        for (const file of fileData) {
            if (file.size >= 5000000) {
                uploadErrors.push({ file, error: t("PT_FILE_SIZE_EXCEEDS_5MB") });
    ...
    const handleSpecialCategoryProofUpload = async (fileData) => {
        const uploadErrors = [];
        for (const file of fileData) {
            if (file.size >= 5000000) {
                uploadErrors.push({ file, error: t("PT_FILE_SIZE_EXCEEDS_5MB") });
        ...
        if (onSelect && config?.key) {
            ...
            formValue.validate = () => {
                ...
                documents.forEach((doc, index) => {
                    const docLabel = index === 0 ? "Address Proof" : index === 1 ? "Identity Proof" : index === 2 ? "Registration Proof" : index === 3 ? "Usage Proof" : "Special Category Proof";
    ...
    return (
        <div>
            <Card>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                        <LabelFieldPair vertical={true} removeMargin={true}>
                            <HeaderComponent className="label" styles={{ margin: "0rem" }}>
                                ...
                                >
                                    ...
                                    {registrationProofDoc?.required && <div style={{ color: "#B91900" }}>{" * "}</div>}
                                </div>
                            </HeaderComponent>
                            <Dropdown
                                option={registrationProofDocuments}
                                selected={registrationProofDocuments.find((opt) => opt.code === documents[2]?.documentCode)}
                        ...
                        <LabelFieldPair vertical={true} removeMargin={true}>
                            ...
                            <FileUpload
                                ...
                                accept=".pdf,.png,.jpg,.jpeg"
                                showAsTags={true}
                                onUpload={handleRegistrationProofUpload}
                ...
                {/* Document 4: Usage Proof */}
                <Card style={{ marginBottom: "1.5rem" }} type={"secondary"}>
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <span style={{ fontSize: "16px", fontWeight: 600, marginRight: "0.5rem" }}>3</span>
                        <span style={{ fontSize: "16px", fontWeight: 600 }}>{t("PT_USAGE_PROOF")}</span>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                        <LabelFieldPair vertical={true} removeMargin={true}>
                            <HeaderComponent className="label" styles={{ margin: "0rem" }}>
                                ...
                                >
                                    ...
                                    {usageProofDoc?.required && <div style={{ color: "#B91900" }}>{" * "}</div>}
                                </div>
                            </HeaderComponent>
                            <Dropdown
                                option={usageProofDocuments}
                                selected={usageProofDocuments.find((opt) => opt.code === documents[3]?.documentCode)}
                                select={(value) => handleDocumentCodeChange(3, value)}
                                optionKey="name"
                                t={t}
                                placeholder={t("PT_SELECT_DOCUMENT")}
                            />
                        ...
                        <LabelFieldPair vertical={true} removeMargin={true}>
                            ...
                            <FileUpload
                                id={`usage-proof-upload`}
                                variant="uploadField"
                                accept=".pdf,.png,.jpg,.jpeg"
                                showAsTags={true}
                                onUpload={handleUsageProofUpload}
                                removeTargetedFile={() => handleFileRemove(3)}
                                multiple={false}
                            />
                ...
                {/* Document 4: Special Category Proof */}
                <Card style={{ marginBottom: "1.5rem" }} type={"secondary"}>
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <span style={{ fontSize: "16px", fontWeight: 600, marginRight: "0.5rem" }}>3</span>
                        <span style={{ fontSize: "16px", fontWeight: 600 }}>{t("PT_SPECIAL_CATEGORY_PROOF")}</span>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                        <LabelFieldPair vertical={true} removeMargin={true}>
                            <HeaderComponent className="label" styles={{ margin: "0rem" }}>
                                ...
                                >
                                    ...
                                    {specialCategoryProofDoc?.required && <div style={{ color: "#B91900" }}>{" * "}</div>}
                                </div>
                            </HeaderComponent>
                            <Dropdown
                                option={specialCategoryProofDocuments}
                                selected={specialCategoryProofDocuments.find((opt) => opt.code === documents[3]?.documentCode)}
                                select={(value) => handleDocumentCodeChange(3, value)}
                                optionKey="name"
                                t={t}
                                placeholder={t("PT_SELECT_DOCUMENT")}
                        ...
                        <LabelFieldPair vertical={true} removeMargin={true}>
                            ...
                            <FileUpload
                                ...
                                accept=".pdf,.png,.jpg,.jpeg"
                                showAsTags={true}
                                onUpload={handleSpecialCategoryProofUpload}
                                removeTargetedFile={() => handleFileRemove(4)}
                                multiple={false}
                            />
                            {documents[4]?.fileName && (
                                <div style={{ marginTop: "0.5rem", color: "#0B4B66", fontSize: "14px" }}>
                                    ✓ {documents[3].fileName}
                                </div>
                            )}
