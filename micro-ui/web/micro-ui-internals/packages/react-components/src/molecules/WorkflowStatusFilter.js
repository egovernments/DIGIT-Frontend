      ...
      {statusMap?.map((row) => {
        return (
          <CheckBox
            ...
            label={`${t(Digit.Utils.locale.getTransformedLocale(populators?.onlylabelPrefix ? `${populators.labelPrefix}${row?.state}` : `${populators.labelPrefix}${row?.businessService}_STATE_${row?.state}`))} (${row?.count || 0})`}
          />
        );
      })}
    </>
