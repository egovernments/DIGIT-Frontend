...
export const FormComposer = (props) => {
            ...
            return (
              <React.Fragment key={index}>
                <div style={field.isInsideBox ? getCombinedStyle(field?.placementinbox) : {}}>
                  ...
                  {/* {index > 0 && <Button onButtonClick={() => handleDeleteField(index)} label={"Delete"} variation="danger" type="button" />} */}
                </div>
              </React.Fragment>
            );
          return (
  ...
  const renderFormFields = (props, section, index, array, sectionFormCategory) => (
    <React.Fragment key={index}>
      ...
      {props.heading && (
        <CardSubHeader className={props?.cardSubHeaderClassName ? props?.cardSubHeaderClassName : ""} style={{ ...props.headingStyle }}>
          {" "}
          {props.heading}{" "}
        </CardSubHeader>
      )}
      ...
      {props.submitInForm && (
        <SubmitBar
          ...
          submit="submit"
          disabled={isDisabled}
          className={`w-full ${props?.buttonClassName ? props?.buttonClassName : ""}`}
        />
      )}
